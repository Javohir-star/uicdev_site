...
def create_checkout(data: CheckoutRequest):
    ...
    return {
        ...
        "checkout_url": f"http://localhost:8001/checkout/{session_id}",
    }
...
def get_checkout_page(session_id: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT amount, currency, status FROM payment_sessions WHERE id = ?",
        (session_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def confirm_payment(session_id: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT callback_url FROM payment_sessions WHERE id = ? AND status = 'PENDING'",
        (session_id,),
    )
    row = cursor.fetchone()
    ...
    if not row:
        ...
        raise HTTPException(
            status_code=404,
            detail="Pending payment session not found or already processed.",
        )

    callback_url = row[0]

    cursor.execute(
        "UPDATE payment_sessions SET status = 'SUCCESS' WHERE id = ?", (session_id,)
    )
    conn.commit()
    conn.close()
